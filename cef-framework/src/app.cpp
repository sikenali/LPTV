...
// ---- Ref count implementations ----
static void my_base_add_ref(cef_base_ref_counted_t* self) {
  (void)self;
}

static int my_base_release(cef_base_ref_counted_t* self) {
  (void)self;
  return 0;
}

static int my_base_has_one_ref(cef_base_ref_counted_t* self) {
  (void)self;
  return 1;
}
...
static void my_app_on_before_command_line_processing(
  ...
  cef_string_utf8_t ws_name, ws_val;
  cef_string_utf8_set("window-size", 11, &ws_name, 1);
  cef_string_utf8_set("1920,1080", 11, &ws_val, 1);
  command_line->append_switch_with_value(command_line, reinterpret_cast<const cef_string_t*>(&ws_name), reinterpret_cast<const cef_string_t*>(&ws_val));
  cef_string_utf8_clear(&ws_name);
  cef_string_utf8_clear(&ws_val);
...
static void my_app_init(my_app_t* app) {
  ...
  // Set up base ref counted structure
  app->base.base.size = sizeof(cef_base_ref_counted_t);
  app->base.base.add_ref = my_base_add_ref;
  app->base.base.release = my_base_release;
  app->base.base.has_one_ref = my_base_has_one_ref;
