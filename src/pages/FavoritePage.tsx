...
const FavoritePage: React.FC = () => {
  ...
  const favChs = useMemo(() =>
    allChannels.filter(ch => favorites.includes(`${ch.tid}-${ch.id}`)),
    [favorites]
  );
  ...
  const filteredFavs = useMemo(() => {
    ...
    return favChs.filter(ch => ch.name.toLowerCase().includes(q) || ch.category.includes(q));
  }, [favChs, searchQuery]);

  const groupedFavs = useMemo(() => {
    const map = new Map<string, IptvChannel[]>();
    for (const ch of filteredFavs) {
      if (!map.has(ch.category)) map.set(ch.category, []);
      map.get(ch.category)!.push(ch);
    }
    return Array.from(map.entries()).map(([group, channels]) => ({ group, channels }));
  }, [filteredFavs]);
                            ...
                            <div className="relative h-[120px] flex items-center justify-center" style={{ background: isBlack ? '#1a1a1a' : '#f8f3e8' }}>
                              ...
                              <div className="absolute top-2 right-2">
                                <button
                                  ...
                                  className="w-7 h-7 rounded-full bg-black/50 hover:bg-black/70 flex items-center justify-center transition-colors cursor-pointer"
                                >
                                  <RiHeartFill className="w-3.5 h-3.5 text-red-500" />
                                </button>
                              </div>
                            </div>
                            <div className="p-3">
